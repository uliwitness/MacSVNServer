...
-(id)   init
{
	...
	if( self )
	{
		if( !gUKKQueueSharedNotificationCenterProxy )
			gUKKQueueSharedNotificationCenterProxy = [[[NSWorkspace sharedWorkspace] notificationCenter] copyMainThreadProxy];
...
-(void) removePathFromQueue: (NSString*)path
{
    ...
    AT_SYNCHRONIZED( self )
    {
        ...
        if( index == NSNotFound )
        {
			NSLog( @"removePathFromQueue: Couldn't find path %@ to unsubscribe.", path );
		    return;
		}
			...
			if( n > 0 )
			{
				if( ev.filter == EVFILT_VNODE )
				{
					if( ev.fflags )
					{
						...
						NSLog( @"newpath: %@", [self filePathForFileDescriptor: fd oldPath: fpath] );*/
...
-(void) postNotification: (NSString*)nm forFile: (NSString*)fp
{
	...
	if( !delegateProxy || alwaysNotify )
	{
		...
		[[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName: nm object: fp];	// Deprecated. Not thread-safe and useless info in notification, hard to subscribe to selectively.
		#else
